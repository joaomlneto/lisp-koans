...
(define-test test-push-pop
    (let ((stack '(10 20 30 40))
      ...
      (assert-equal (list 20 30 40) stack)))
...
(define-test test-append
  ...
  (let ((abc '(:a :b :c))
    ...
    (assert-equal (list :a :b :c) abc)
    (assert-equal (list :x :y :z) xyz)
    (assert-equal (list :a :b :c :x :y :z) abcxyz)))
...
(define-test test-list-breakdown
  ...
  (assert-equal (list 2 3) (cdr '(1 2 3)))
